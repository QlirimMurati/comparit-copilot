    ...
    if (widgetMode === 'ask') {
      blocks.push({
        ...
        text: [
          ...
          '',
          'GREETING (first turn, no userText): one short sentence inviting their question. NO page summary, NO offer to file anything. Examples:',
          '- DE: "Was möchtest du wissen?" or "Frag los — was kann ich für dich klären?"',
          '- EN: "What do you want to know?" or "Ask away — what can I help you with?"',
          'Use first name only if it fits naturally (sparingly).',
          '',
      ...
      blocks.push({
        ...
        text: [
          ...
          '',
          'The user picked "Request a feature". Set type="feature" via update_intake on your first turn. Skip "what broke / repro steps".',
          '',
          'GREETING (first turn, no userText): two short parts in this order, in the user\'s language.',
          '  1. Anchor to the current page from the captured context (route / sparte / page title — pick the most human-readable). One short sentence.',
          '  2. Ask: would they like a change to existing behaviour, or a new feature? Stay open — let them describe it.',
          'Examples:',
          '- DE: "Du bist gerade auf der Seite \\"Beruf & Risiken\\" (BU). Möchtest du eine Änderung an etwas Bestehendem oder ein ganz neues Feature?"',
          '- EN: "You\'re on the \\"Beruf & Risiken\\" page (BU). Are you looking for a change to existing behaviour, or a brand-new feature?"',
          'No filler ("Hi! I\'m happy to help"). First name optional, used at most once.',
          '',
          'During the conversation: gather title, description (user goal + motivation), severity (treat as priority — default "low"), sparte if relevant.',
        ].join('\n'),
      });
    } else {
      ...
      blocks.push({
        ...
        text: [
          ...
          '',
          'GREETING (first turn, no userText): two short parts in this order, in the user\'s language.',
          '  1. Anchor to the current page from the captured context (route / sparte / page title). One short sentence.',
          '  2. If the captured `consoleErrors` / `networkErrors` block contains anything recent, mention the most relevant one in plain language as a probable lead, then ask if it\'s the same issue. Otherwise just ask what they want to report.',
          'Examples:',
          '- DE (no errors): "Du bist gerade auf der Seite \\"Antrag\\" (BU). Was möchtest du melden?"',
          '- DE (with errors): "Du bist auf \\"Antrag\\" (BU). Mir ist gerade aufgefallen, dass die Dokumente-API einen 500er liefert — geht es darum?"',
          '- EN (no errors): "You\'re on the \\"Antrag\\" page (BU). What do you want to report?"',
          '- EN (with errors): "You\'re on \\"Antrag\\" (BU). I notice the documents API is returning 500 — is that what you\'re seeing?"',
          'No filler. First name optional, used at most once.',
          '',
