...
- **Active workstream:** _all 16 unblocked workstreams shipped: W1, W2, W3, W4, W5, W6, W7, W8, W9, W10, W11, W12, W13, W14, W15, W17._ Only W16 (Confluence Q&A) remains — paused per user, not blocked.
...
| W7 | Jira MCP integration | Done (2026-04-28) | Read-only by design; push-to-Jira gated by preview/confirm hash; tenant `comparit.atlassian.net`, project `LV` |
...
| W9 | Code-localizer agent | Done (2026-04-28) | `POST /api/reports/:id/localize` runs Claude with semantic_search/grep/find_symbol/read_chunk tools; persists to `aiProposedTicket.localization` |
...
| W16 | Confluence Q&A bot | PAUSED | User-mandated pause 2026-04-28 — do not implement until lifted |
| W17 | Cross-source dedup (reports ↔ tickets) | Done (2026-04-28) | `POST /api/reports/check-duplicate` now returns `{ similarReports, similarTickets, candidates }`; tickets embedded during sync |
...
- 2026-04-28 — W17 cross-source dedup (reports ↔ Jira tickets) — `TicketsCacheService` now embeds tickets via `voyage-3` (general text model, same dimension as `bug_reports.embedding` so vectors are directly comparable) during the paged sync. Embedding text composes `summary + description + labels + components + status + type`. Existing rows whose summary/description change get their embedding nulled out so the next sync re-embeds. New helper `embedAllMissing()` for one-time backfill. `DedupService.checkDuplicateAcrossSources` runs ONE Voyage embed for the query then queries both `bug_reports.embedding` and `tickets_cache.embedding` in parallel, returning `{ similarReports, similarTickets }`. `POST /api/reports/check-duplicate` updated to return `{ similarReports, similarTickets, candidates }` (the `candidates` alias kept temporarily for backward-compat with existing W4 consumers — TriageAgent still uses the original single-source `checkDuplicate` method, no breaking change). Verified: api build green; 69/69 jest tests passing including 2 new for `checkDuplicateAcrossSources` (happy path with 2-source ceiling filter + unconfigured 503).
- 2026-04-28 — W7 Jira MCP integration — implemented as read-only-by-design per the user-mandated Jira write-safety constraints. New table `tickets_cache` (migration `0010`). `JiraClient` is a thin REST wrapper that exposes ONLY `whoami`, `searchByJql`, `getIssue`, `createIssue` — no DELETE, no PATCH/PUT, no transitions, no comment edits, no relinks. `JqlBuilderService` composes JQL with hardcoded scoping to `JIRA_ALLOWED_PROJECTS` and rejects raw clauses that reference projects outside the allowlist or contain forbidden keywords (`delete`, `drop`, `update`, `insert`) or semicolons. `TicketsCacheService` upserts issues from JQL paginated search; `JiraSyncQueueService` runs an hourly delta sync via BullMQ (cron-style) plus full-sync trigger. Endpoints (JWT-guarded): `POST /api/jira/search`, `GET /api/jira/issue/:key`, `GET /api/jira/whoami`, `POST /api/jira/sync/delta` (admin/qa_lead), `POST /api/jira/sync/full` (admin), `GET /api/jira/cache/:key`. **Push-to-Jira is a strict two-step flow:** `POST /api/reports/:id/push-to-jira/preview` returns the prepared payload + a SHA-256 `previewHash` over the canonicalised payload (no Jira call), and `POST /api/reports/:id/push-to-jira/confirm` requires the same `previewHash` and only THEN calls `createIssue`. Hash mismatch ⇒ 400. Already-linked report ⇒ 400 (refuses double-push). On success, persists `bug_reports.jira_issue_key` and mirrors the new issue into `tickets_cache`. Verified: api build green; 67/67 jest tests passing including 14 new across `jql-builder.spec` (10 cases including injection-shape rejection) and `push-to-jira.spec` (6 cases including hash mismatch + already-linked rejection). Live endpoints return graceful 503 with descriptive message when env vars missing.
